...
function gigpress_prepare_show_fields($context = 'new') {
	...
	$show['show_status_text'] = gigpress_db_in($_POST['show_status_text']);

	$show['show_tour_attribute'] = gigpress_db_in($_POST['show_tour_attribute']);
	...
	if($_POST['show_venue_id'] == 'new') {
		...
		$format = array('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%f', '%f');
		$insert_venue = $wpdb->insert(GIGPRESS_VENUES, $venue, $format);
...
function gigpress_prepare_venue_fields() {
	...
	$venue = array(
		...
		'venue_not_gigmapped' => gigpress_db_in($_POST['venue_not_gigmapped']),
		'venue_lat' => NULL,
		'venue_lng' => NULL
	);
	
	if ( !empty($gpo['overview_map']) ) { 
		if ( $venue['venue_not_gigmapped'] == 0) {     // venue not excluded from gigmap
			...
			$loc = geocoder::getLocation($address, $venue['venue_country']);
			if ( $loc != FALSE ) { //success!
				$venue['venue_lat'] = $loc[lat];
				$venue['venue_lng'] = $loc[lng];
			}
		}		// if ( empty($venue->venue_not_gigmapped))   
	}
	return $venue;
}
...
function gigpress_add_show() {
	...
	if($errors) {
		...
		$format = array('%s', '%s', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%d');
				...
				if(!empty($show['artist_id']))
				{
					...
					if(empty($venue_exists)) 
					{
						...
						$new_venue = array(
							...
							'venue_not_gigmapped' => gigpress_db_in(@$show['Venue not gigmapped']),
							'venue_lat' => gigpress_db_in(@$show['Venue lat']),
							'venue_lng' => gigpress_db_in(@$show['Venue lng'])
						);
						
						if ( !empty($gpo['overview_map']) and empty( $new_venue['venue_not_gigmapped'] ) and empty( $new_venue['venue_lat'] )) {   //csv contains no geodata. Create them
						
							$address=$new_venue['venue_address'].",".$new_venue['venue_city'];
							if ( !empty($new_venue['venue_state'])) { $address = $address."(".$new_venue['venue_state'].")";};
							$address = $address.",".$new_venue['venue_postal_code'];
									
							$loc = geocoder::getLocation($address, $new_venue['venue_country']);
							if ( $loc != FALSE ) { //success!
								$new_venue['venue_lat'] = $loc[lat];
								$new_venue['venue_lng'] = $loc[lng];
							}
						}		// create geodata  
					...
					if(!empty($show['venue_id']))
					{
						...
						if($wpdb->get_var(
							...
							$new_show = array(
								...
								'show_related' => '0',
								'show_status_text' => gigpress_db_in(@$show['Status Text']),
								'show_tour_attribute' => gigpress_db_in(@$show['Tour attribute']),
							);
							...
							$format = array('%s','%s','%d','%s','%d','%d','%d','%s','%s','%s','%s','%s','%s','%s','%d','%s','%s');
